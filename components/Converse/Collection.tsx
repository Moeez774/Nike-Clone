...
const Collection = ({ allData, hideFilters, showFilters, setOpen }: { allData: any[], hideFilters: Dispatch<SetStateAction<boolean>>, showFilters: boolean, setOpen: Dispatch<SetStateAction<boolean>> }) => {
    ...
    return (
        <div className='flex flex-col gap-2 mdx:gap-6'>
            ...
            <div className='flex gap-8 relative z-30 justify-end py-2' style={{ fontWeight: '500' }}>
                <div>
                    ...
                    <div className='flex mdx:hidden items-center py-1 px-6 justify-between w-screen'>
                        {/* results */}
                    <div>
                        <h1 className='text-[gray]'>{allData.length+allData.length} Results</h1>
                    </div>
            ...
            <div className='grid grid-cols-2 mdx:grid-cols-3 gap-2 mdx:gap-4'>
                {allData.map((e, index) => {
                    return (
                        <div className='flex cursor-pointer mb-16 flex-col gap-4' key={index}>
                            ...
                            <div className='font-semibold relative mx-4 mdx:mx-0 z-30 flex flex-col gap-1'>
                                <h1>{e.name}</h1>
                                <h1 className='text-[gray]'>{e.category}</h1>
                                <h1 className='text-[gray]'>{e.color}</h1>
                            </div>

                            <div className='font-semibold mx-4 mdx:mx-0 relative z-30'>
                                <h1 className='text-black'>{e.price}</h1>
                            </div>
